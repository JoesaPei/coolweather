...
/**
 * Created by peizhuo on 2018/5/15.
 * 解析和处理服务器返回的数据，并将实体类数据保存到数据库LitePal
 */

public class HandleWebData {
        ...
        if(!TextUtils.isEmpty(response)){
            try{
                ...
                for(int i = 0; i < allProvinces.length(); i++){
                    ...
                    province.save();// 直接将实体类数据保存到数据库LitePal
                }
                return true;
            } catch (Exception e){
                e.printStackTrace();
        ...
        if(!TextUtils.isEmpty(response)){
            try{
                ...
                for(int i = 0; i < allCities.length(); i++){
                    ...
                    city.save();// 直接将实体类数据保存到数据库LitePal
                }
                return true;
            } catch (Exception e){
                e.printStackTrace();
        ...
        if(!TextUtils.isEmpty(response)){
            try{
                ...
                for(int i = 0; i < allCounties.length(); i++){
                    ...
                    county.save();// 直接将实体类数据保存到数据库LitePal
                }
                return true;
            } catch (Exception e){
                e.printStackTrace();
